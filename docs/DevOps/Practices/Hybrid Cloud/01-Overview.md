...
1. **Luồng CI/CD:** Dev push Code lên GitLab, Gilab trigger **Jenkins** qua webhook để build pipeline và update image trong harbor registry và update manifest on-premise, sau đó thông báo tới telegram cho QA vào test, sau khi QA test xong sẽ nhấn approve để push lên Artifact Registry
